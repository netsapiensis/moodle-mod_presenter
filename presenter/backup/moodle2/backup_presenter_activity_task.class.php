...
require_once($CFG->dirroot . '/mod/presenter/backup/moodle2/backup_presenter_stepslib.php');

class backup_presenter_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        $this->add_step(new backup_presenter_activity_structure_step('presenter structure', 'presenter.xml'));
    }
    ...
    static public function encode_content_links($content) {
        ...
        $base = preg_quote($CFG->wwwroot . '/mod/presenter', '#');

        // Provides the interface for overall authoring of lessons
        $pattern = '#' . $base . '/edit\.php\?id=([0-9]+)#';
        $replacement = '$@PRESENTEREDIT*$1@$';
        $content = preg_replace($pattern, $replacement, $content);

        // This page lists all the instances of presenter in a particular course
        $pattern = '#'.$base.'/index\.php\?id=([0-9]+)#';
        $replacement = '$@PRESENTERINDEX*$1@$';
        $content = preg_replace($pattern, $replacement, $content);

        // This prints a particular chapter of presenter
        $pattern = '#'.$base.'/view\.php\?id=([0-9]+)&(amp;)?chapterid=([0-9]+)#';
        $replacement = '$@PRESENTERVIEWCHAPTER*$1*$2@$';
        $content = preg_replace($pattern, $replacement, $content);

        // This prints a particular chapter of presenter with optional param open
        $pattern = '#'.$base.'/view\.php\?open=([0-9]+)&(amp;)?id=([0-9]+)&(amp;)?chapterid=([0-9]+)#';
        $replacement = '$@PRESENTERVIEWCHAPTEROPEN*$1*$3@$';
        $content = preg_replace($pattern, $replacement, $content);

        // Link to one lesson by cmid
        $pattern = '#'.$base.'/view\.php\?id=([0-9]+)#';
        $replacement = '$@PRESENTERVIEWBYID*$1@$';
        $content = preg_replace($pattern, $replacement, $content);
