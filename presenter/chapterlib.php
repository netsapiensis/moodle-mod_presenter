...
function get_next_chapter_id($chapter) {
	...
	if (!empty($ch->id) && $ch->id != $chapter->id) {
		return $ch->id;
	} else {
		return false;
	}
...
function get_course_module($id, $course) {
	global $CFG;
    return get_record("course_modules", "course", $course, "instance", $id);
}
...
function chapter_completed($chapter_id, $username) {
	...
	global $CFG;
	$all = get_records_select('presenter_chapters_users', "username='$username' AND chapter_id='$chapter_id'", "username");
	
	return !empty($all) && !empty($all[0]->username) ? $all[0]->username : '';
}
