 ...
 */
    ...
    $course     = $SESSION->mod_presenter_course;
    $cm         = $SESSION->mod_presenter_cm;
    $context    = $SESSION->mod_presenter_context;
    $chapter    = $SESSION->mod_presenter_chapter;
    $filename   = $SESSION->mod_presenter_filename;
	...
	if (isset($_SERVER['PATH_INFO']) || isset($_GET[XMOOV_GET_FILE]))
	{
        ...
        $pattern = "/" . basename($_SERVER['SCRIPT_NAME']) . "/";
        
		if(strrchr($fileName, '.') == '.flv' && strlen($fileName) > 2 && !preg_match($pattern, $fileName))
		{
            $fileObj = mod_presenter_get_video_file_obj($course, $cm, $context, $chapter, $fileName) or die ('<b>ERROR:</b> xmoov-php could not open (' . $fileName . ')');
            $fh = $fileObj->get_content_file_handle();
            	
			$fileSize = $fileObj->get_filesize() - (($seekPos > 0) ? $seekPos  + 1 : 0);
			...
			while(!feof($fh)) 
			{
				...
				else
				{
                    ...
                    print (fread($fh, $fileObj->get_filesize()));
				}
			}
