...
//-----------------javascript for adding multiple chapters-----------------------
        ...
        $showThis = '<input class="flag" type="image" src="'. $CFG->pixpath . '/i/one.gif" style="float: right; margin-right: 17px;" title="Show only this chapter" onclick="showOnly(this);return false" />';
        $showAll = '<input type="image" src="'. $CFG->pixpath . '/i/all.gif" style="display: none; float: right; margin-right: 17px;" title="Show all chapters" onclick="showAll(this);return false;" />';
        
        $repeatarray[] = &$mform->createElement('html', $showThis);
        $repeatarray[] = &$mform->createElement('html', $showAll);
        
        $style = 'float: right; background: url(' . $CFG->pixpath . '/t/up.gif);padding:0;display: block;width: 11px; height: 11px;border: 0;cursor: pointer; margin-right: 20px;margin-top: 5px;';
        $style1 = 'float: right; background: url(' . $CFG->pixpath . '/t/down.gif);padding:0;display: block;width: 11px; height: 11px;border: 0;cursor: pointer; margin-right: 20px;margin-top: 5px;';
        $style2 = 'float: right; background: url(' . $CFG->pixpath . '/t/delete.gif);padding:0;display: block;width: 11px; height: 11px;border: 0;cursor: pointer; margin-right: 20px;margin-top: 5px;';
...
// buttons
    ...
    {
        global $CFG;
        return '<style type="text/css">' . file_get_contents($CFG->dirroot . "/mod/presenter/style_presenter.css") . '</style>';
    }
}
?>
